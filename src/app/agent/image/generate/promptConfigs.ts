...
- other：其他未归类意图。`,
            ...
            image_prompt: {
                role: '你是日语学习插画提示设计师。',
                task: '基于选定意图，规划 2~5 张教学插画提示并输出结构化 JSON。',
                steps: [
                    '首先根据意图规划需要哪些主题的插画。各图主题可以参考后面的说明（比如语法的定义/比如常用例句/或相关性语法或者单词的对比/同类型词汇等）',
                    '然后根据这些主题插画，设定一个相对统一的画面风格，用来描述各个主题的画面'
                ],
                description: '严格 JSON 输出，字段：title, description, prompt, text。' +
                    'title：表示插画的主题，尽量简介且能包含插画的主要内容； description：稍微详细的描述插画的内容；' +
                    'prompt：用于描述插画的信息的，可以用来直接发给大模型用来生成插画（不要引导在插画里生成文字）；' +
                    'text：用来记录关键的语法，例句，单词等跟插画息息相关的信息。',
                notes: [
                    'images 数量 2~5张',
                    '主要prompt字段里一定不要让模型在生成插画的时候输出文字。详细的跟文字相关的信息放到text里',
                ]
            }
        }
    }
