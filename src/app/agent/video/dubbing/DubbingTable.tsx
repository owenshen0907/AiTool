...
export default function DubbingTable(props: Props) {
    /** ------------ props 解构 ------------ */
    const {
        cases, selectedIds, remaining, batchConcurrency, voiceTones,
        ...
        globalSupplierId, exportTemplate, exportWithData, parseImportFile,
    } = props;
    ...
    useEffect(()=>{
        setRange(cases.length ? { start: 1, end: cases.length } : { start: 0, end: 0 });
    }, [cases.length]);
    ...
    const startMergedPlay = async () => {
        ...
        if (!urls.length) { alert('区间内暂无音频'); return; }
    ...
    useEffect(()=>()=>{ if (merged) revokeMergedAudio(merged.blobUrl); },[merged]);
    ...
    const columns: ColumnDef<CaseImportRow>[] = [{ key:'text', label:'Case 文本', editable:true }];
        ...
        <div className="flex-1 overflow-auto w-full">
            ...
            <div className="border border-gray-200 rounded-md p-4 mb-4 bg-gray-50">
                <div className="flex items-center flex-wrap gap-2">
                    ...
                    {/* 新增行 + Import */}
                    <button onClick={onAddCase} className="px-3 py-1 bg-gray-700 text-white rounded">新增行</button>
                    <div className="relative">
                        <button onClick={()=>setShowImport(v=>!v)} className="p-2 hover:bg-gray-100 rounded"><MoreVertical size={18}/></button>
                        {showImport && (
                            <div className="absolute z-10 mt-1 bg-white border rounded shadow-lg p-3">
                                <DataImportManager<CaseImportRow>
                                    ...
                                    exportTemplate={exportTemplate} exportWithData={exportWithData}
                                    parseImportFile={parseImportFile} columns={columns}
                                    onImportConfirmed={(rows)=>{ onImport(rows); setShowImport(false);} }
                                />
                            </div>
                        )}
                    </div>
                    ...
                    {/* 批量执行区 */}
                    <button onClick={onBatchGenerate} className="px-3 py-1 bg-green-500 text-white rounded">一键执行</button>
                    <span className="text-sm text-gray-600">已选 {selectedIds.length}，剩余 {remaining}</span>
                    ...
                    <input type="number" min={0} max={cases.length} value={range.start}
                           onChange={e=>setRange(r=>({...r,start:+e.target.value}))}
                           className="w-14 border rounded p-1 text-sm"/>
                    <span className="text-gray-500">-</span>
                    <input type="number" min={0} max={cases.length} value={range.end}
                           onChange={e=>setRange(r=>({...r,end:+e.target.value}))}
                           className="w-14 border rounded p-1 text-sm"/>

                    <button onClick={handleTogglePlay}
                            className={`px-3 py-1 rounded ${playing?'bg-red-500':'bg-indigo-500'} text-white`}>
                        {playing?'暂停':'合成播放'}
                    </button>

                    {merged && (
                        <>
                            <audio ref={audioRef} src={merged.blobUrl} controls autoPlay
                                   className="w-64 ml-2"
                                   onTimeUpdate={onTimeUpdate}
                                   onEnded={()=>setPlaying(false)}/>
                            <span className="ml-2 text-sm text-gray-700">{playing?`正在播放第 ${curIdx} 条`:''}</span>
                        </>
                    )}
                    ...
                    <button
                        onClick={()=>exportAudioZip(
                            cases,
                            selectedIds.map(id=>cases.findIndex(c=>c.id===id)).filter(i=>i>=0).sort((a,b)=>a-b),
                            voiceTones,
                            globalSupplierId
                        )}
                        className="ml-4 px-3 py-1 bg-blue-600 text-white rounded">导出音频片段及字幕</button>
            ...
            <table className="min-w-full table-auto border-collapse">
                <colgroup>
                    <col style={{width:'4%'}}/><col style={{width:'5%'}}/><col style={{width:'28%'}}/>
                    <col style={{width:'10%'}}/><col style={{width:'5%'}}/><col style={{width:'5%'}}/>
                    <col style={{width:'5%'}}/><col style={{width:'20%'}}/><col style={{width:'5%'}}/>
                ...
                <tr className="bg-gray-100">
                    ...
                    <th className="border p-2 text-center">
                        <input
                            type="checkbox"
                            checked={pagedCases.length>0 && pagedCases.every(c=>selectedIds.includes(c.id))}
                            onChange={e=>{
                                const ids = pagedCases.map(c=>c.id);
                                onSelectIds(e.target.checked
                                    ? Array.from(new Set([...selectedIds, ...ids]))
                                    : selectedIds.filter(id=>!ids.includes(id)));
                            }}
                        />
                    </th>
                    {['Case 文本','音色','格式','速率','音量','音频','建连','完成','操作']
                        .map(t=><th key={t} className="border p-2 text-center">{t}</th>)}
                </tr>
                </thead>
                <tbody>
                    ...
                    onChange={e=>onUpdateCase(c.id,{text:e.target.value})}/>
                            ...
                            <td className="border p-2 text-center">
                                <select className="w-full border p-1 rounded"
                                    ...
                                    <option value="">--</option>
                                    {voiceTones.filter(v=>v.supplierId===globalSupplierId)
                                        .map(v=><option key={v.id} value={v.toneId}>{v.name}</option>)}
                                </select>
                            </td>
                            ...
                            {/* 音量 */}
                            <td className="border p-2 text-center">
                                <input type="number" step="0.1" min="0.1" max="2.0"
                            ...
                            <td className="border p-2 flex justify-center space-x-1">
                                <button onClick={()=>onGenerateCase(c)} className="px-2 py-1 bg-green-500 text-white rounded">生成</button>
                                <button onClick={()=>onRemoveCase(c.id)} className="px-2 py-1 bg-red-500 text-white rounded">删除</button>
                            </td>
                        </tr>
                    );
                })}
