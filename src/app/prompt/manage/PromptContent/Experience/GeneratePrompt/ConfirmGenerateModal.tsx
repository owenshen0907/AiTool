...
export default function ConfirmGenerateModal({ isOpen, onClose, data }: Props) {
    ...
    const buildMessages = async () => {
        ...
        switch (data.step0) {
            ...
            case 'VIDEO_TEXT':
                parts.push({ type: 'video_url', video_url: { url: toPreview(data.step1 as string), detail: 'high' } });
                break;
            case 'AUDIO_TEXT':
                parts.push({ type: 'input_audio', input_audio: { data: data.step1 as string, format: 'mp3', detail: 'high' } });
                break;
        }
        ...
        <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-auto">
            ...
            <div className="relative w-[860px] max-w-full bg-white rounded-lg shadow-xl mt-10 p-6 z-10">
                ...
                <div className="space-y-4 max-h-[340px] overflow-auto border p-4 rounded">
                    ...
                    {data.step3.fmt === 'JSON' && <pre className="bg-gray-50 p-2 rounded text-xs overflow-auto">{data.step3.schema}</pre>}
                    <p><b>⑤ 输出示例：</b>{data.step4 || '（无）'}</p>
                </div>
                <textarea rows={3} className="w-full mt-4 p-2 border rounded" placeholder="补充说明（可选，将作为额外 user 消息）" value={extra} onChange={e => setExtra(e.target.value)} />
                <button onClick={handleGenerate} disabled={loading} className="mt-4 px-5 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50">{loading ? '生成中…' : '生成 Prompt'}</button>
                {thinking && <pre className="mt-5 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm whitespace-pre-wrap">🧠 <b className="text-yellow-700">深度思考：</b>{`\n`}{thinking}</pre>}
                {answer && <div className="relative mt-4"><button onClick={() => navigator.clipboard.writeText(answer)} className="absolute top-0 right-0 text-gray-500 hover:text-gray-800" title="复制"><Copy size={16}/></button><pre className="p-3 bg-gray-100 rounded max-h-[260px] overflow-auto text-sm whitespace-pre-wrap">{answer}</pre></div>}
            </div>
        </div>
    );
}